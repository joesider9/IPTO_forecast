...
class Objective(object):
    ...
    def __call__(self, trial):
        ...
        mask_train, mask_val = train_test_split(y.index, test_size=self.static_data['val_test_ratio'],
                                                random_state=42,
                                                stratify=y.values)
        mask_train, mask_test = train_test_split(mask_train, test_size=self.static_data['val_test_ratio'],
                                                 random_state=42,
                                                 stratify=y.loc[mask_train].values)
        cv_masks = [mask_train, mask_val, mask_test]
        ...
        if 'bootstrap_type' in experiment_params.keys():
            if experiment_params["bootstrap_type"] == "Bayesian":
                experiment_params["bagging_temperature"] = trial.suggest_float("bagging_temperature", 0, 10)
            elif experiment_params["bootstrap_type"] == "Bernoulli":
                experiment_params["subsample"] = trial.suggest_float("subsample", 0.1, 1, log=True)
...
def train_classifier(static_data, method, cluster_name, combine_cluster_dir, cluster_dir, best_predictor, predictors_id,
                     refit=False):
