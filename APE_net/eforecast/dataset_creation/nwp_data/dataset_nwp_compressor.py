from eforecast.dataset_creation.nwp_data.nwp_compressors.load_compressor import LoadCompressor
from eforecast.dataset_creation.nwp_data.nwp_compressors.dense_compressor import DenseCompressor
from eforecast.dataset_creation.nwp_data.nwp_compressors.semi_full_compressor import SemiFullCompressor
from eforecast.dataset_creation.nwp_data.nwp_compressors.full_compressor import FullCompressor
...
class DatasetNWPsCompressor:
    ...
    def compress(self):
        if self.compress_method == 'load':
            ...
            nwp_compressed_all, nwp_compressed, nwp_compressed_distributed = compressor.load_compress()
        elif self.compress_method == 'dense':
            compressor = DenseCompressor(self.static_data, self.nwp_data, self.nwp_metadata)
            nwp_compressed_all, nwp_compressed, nwp_compressed_distributed = compressor.dense_compress()
        elif self.compress_method == 'semi_full':
            compressor = SemiFullCompressor(self.static_data, self.nwp_data, self.nwp_metadata)
            nwp_compressed_all, nwp_compressed, nwp_compressed_distributed = compressor.semi_full_compress()
        elif self.compress_method == 'full':
            compressor = FullCompressor(self.static_data, self.nwp_data, self.nwp_metadata)
            nwp_compressed_all, nwp_compressed, nwp_compressed_distributed = compressor.full_compress()
        else:
