...
class FullCompressor:
    ...
    def perform_full_compress(self, i, ax, nwp_data):
        if self.horizon == 0:
            ...
            names, data = self.full_compressor(nwp_data[:, 0, :, :, i], ax)
            ...
            if nwp_data.shape[1] == 1:
                ...
                for hor in range(self.horizon):
                    ...
                    names, data = self.full_compressor(nwp_data[:, hor, :, :, i], ax)
                    data = pd.DataFrame(data, index=self.nwp_metadata['dates'],
                                        columns=[ax_name + '_' + name for name in names])
                    nwp_compressed = pd.concat([nwp_compressed, data], axis=1)
    ...
    def full_compress(self):
        ...
        if len(groups) == 0:
            ...
            for i, ax in enumerate(axis):
                data = self.perform_full_compress(i, ax, self.nwp_data)
                nwp_compressed = pd.concat([nwp_compressed, data], axis=1)
            ...
            for group in groups:
                ...
                for i, ax in enumerate(axis[group_name]):
                    data = self.perform_full_compress(i, ax, self.nwp_data[group_name])
