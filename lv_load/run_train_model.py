...
BACKEND = 'threads'  #command_line, threads
...
def train_proba_model():
    proba_model = ProbaNetwork(static_data, train=True, refit=RECREATE_DATASETS)
    proba_model.fit()
...
if __name__ == '__main__':
    ...
    # try:
    #     predict_methods(train=True, resampling=True)
    #     predict_combine_methods(train=True, resampling=True)
    #     compute_averages_methods(train=True, resampling=True)
    #     predict_combine_models(train=True, resampling=True)
    #     train_proba_model()
    #     eval_proba_model()
    #
    # except Exception as e:
