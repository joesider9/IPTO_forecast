...
class Predictor:
            ...
            for cluster_name, cluster_path in self.clusters.items():
                ...
                for method in self.methods:
                    ...
                    if not os.path.exists(file_regressor):
                        print(f"Cannot find model {method} of cluster {cluster_name}")
                        continue
    ...
    def predict_concat_nets(self):
        ...
        for hor in self.horizon:
            ...
            pred_models = pred_models.clip(0, np.inf)
            pred_models = pred_models.dropna(axis='index')
            dates = pred_models.index.intersection(X_inputs.index)
            pred_models = pred_models.loc[dates]
            data = X_inputs.loc[dates]
            metadata['dates'] = dates
            pred_clusters = dict()
            clusterer = ClusterOrganizer(self.static_data, is_online=self.online, train=self.train)
            activations = None
            if 'clusters' in self.predictions.keys():
                activations = dict()
                for clusterer_method, rules in self.predictions['clusters'].items():
            ...
            if 'models' not in self.predictions.keys():
                self.predictions['models'] = dict()
            self.predictions['models']['concat_net_models'] = network.predict(X, metadata, activations=activations)
            self.save_predictions()
