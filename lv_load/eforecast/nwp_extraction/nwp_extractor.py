            ...
            if nwp['model'] is not None:
                ...
                if nwp_model in self.nwp_extractors.keys():
                    ...
                    if nwp_model == 'skiron' and sys.platform == 'linux':
                        ...
                        nwp_extractor = EcmwfExtractor(self.static_data, id_nwp, dates=self.dates)
                    elif nwp_model == 'gfs':
                        nwp_extractor = GfsExtractor(self.static_data, id_nwp, dates=self.dates)
                    elif nwp_model == 'openweather':
                        pass
                    else:
                        raise ValueError('Cannot recognize nwp model')
                nwp_extractor.extract_nwps(online=self.is_online)
