...
class DeepNetwork:
    ...
    def fit(self, X, y, cv_masks, meta_data, activations=None, gpu_id=0, X_imp=None):
        ...
        self.params['experiment'] = enhance_model_layers_distributed_data(self.what_data, self.params['experiment'])
        self.params['experiment'] = fix_convolutional_names(self.params['experiment_tag'], self.params['experiment'],
                                                            self.conv_dim)
        self.params['experiment'] = enhance_model_layers_multi_output(self.static_data, self.params['experiment_tag'],
                                                                      self.params['experiment'], self.probabilistic)
        ...
        X_train, y_train, group_layers, mask = self.get_slice(X, cv_masks[0], meta_data, y=y, x_imp=X_imp,
                                                              act=activations,
        ...
        with graph_cnn.as_default():
            with tf.device('/device:GPU:' + str(gpu_id)):
                ...
                model_output, model_layers_built, \
                ...
                for epoch in tqdm(range(epochs)):
                    ...
                    if not self.is_global and not self.is_fuzzy:
                        ...
                        if self.is_global and not self.is_fuzzy:
                            train_schedule_global(sess, trainers, batches, x_pl, y_pl, X_train,
                                                  y_train, learning_rate, lr)
                        elif self.is_global and self.is_fuzzy:
                            train_schedule_fuzzy(sess, epoch, trainers, batches, x_pl, y_pl,
                                                 X_train, y_train,
        ...
        with graph_net.as_default():
            with tf.device('/CPU:0'):
                x_pl = create_placeholders(inp_x, self.params, train=False, is_global=self.is_global,
                                           is_fuzzy=self.is_fuzzy)
                model_output, model_layers_built, act_pl, act_nan_err = build_graph(x_pl,
                                                                                    self.model_layers,
                                                                                    self.params,
                                                                                    is_fuzzy=
                                                                                    self.is_fuzzy,
