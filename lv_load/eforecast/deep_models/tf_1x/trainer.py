...
def train_schedule_for_cluster(sess, trainers, batches, x_pl, y_pl, X_train, y_train, learning_rate, lr, warm):
    if warm > 0:
        for _ in range(warm):
...
def train_schedule_fuzzy(sess, epoch, trainers, batches, x_pl, y_pl, X_train, y_train, learning_rate, lr, warm):
    if warm > 0:
        for s in range(warm):
...
def train_schedule_global(sess, trainers, batches, x_pl, y_pl, X_train, y_train, learning_rate, lr):
    # for rule in rules:
