    ...
    if isinstance(data, dict):
        for name in data.keys():
            ...
            if 'act' not in name and 'clustering' not in name:
                x[inp_name] = data[name][batch]
            elif name == 'data_row':
                x['data_row'] = data['data_row'][batch]
            elif name == 'clustering':
                x['clustering'] = data['clustering'][batch]
            elif 'act' in name:
...
def feed_data_eval(data, target=None):
    ...
    if isinstance(data, dict):
        for name in data.keys():
            inp_name = 'input_' + name if name != 'input' else 'input'
            if 'act' not in name and 'clustering' not in name:
                x[inp_name] = data[name]
            elif name == 'data_row':
                x['data_row'] = data['data_row']
            elif name == 'clustering':
                x['clustering'] = data['clustering']
            elif 'act' in name:
                x[name] = data[name]
    else:
        x['input'] = data
