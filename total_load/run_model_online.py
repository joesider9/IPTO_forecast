...
if __name__ == '__main__':
    ...
    print(f'Total load model start at {date}')
    if date_h.hour == 10:
        dates = pd.DatetimeIndex([date])
        if len(dates) > 0:
            print(f'Total load Data are collected  initialize successfully ')

            pred_day_ahead, proba_ahead= predict_intra(dates, 'CatBoost_classifier', model_horizon='day_ahead')
            pred_day_ahead.columns = ['Total_load_day_ahead']
            pred_day_ahead.to_csv(os.path.join(path_pred, f"Total_load_day_ahead_hist_{date.strftime('%d_%m_%Y')}.csv"))
            proba_ahead.to_csv(
                os.path.join(path_pred, f"Proba_Total_load_day_ahead_hist_{date.strftime('%d_%m_%Y')}.csv"))
            pred_intra, proba_intra = predict_intra(dates, 'CatBoost_classifier', model_horizon='intra')
            pred_intra.columns = ['Total_load_intra']
            pred_intra.to_csv(os.path.join(path_pred, f"total_load_Intra_ahead_hist_{date.strftime('%d_%m_%Y')}.csv"))
            proba_intra.to_csv(
                os.path.join(path_pred, f"Proba_total_load_Intra_ahead_hist_{date.strftime('%d_%m_%Y')}.csv"))
    date_h = convert_timezone_dates(pd.DatetimeIndex([date_h]), timezone1='Europe/Athens', timezone2='CET')[0]
    dates_short = [date_h]
    pred_short = predict_intra(pd.DatetimeIndex(dates_short), 'bcp', model_horizon='short_term')
    pred_short.to_csv(os.path.join(path_pred, f"total_load_short_term_{date_h.strftime('%d_%m_%Y %H')}.csv"))
    print(f'total_load Intra-head model predictions are saved successfully')
