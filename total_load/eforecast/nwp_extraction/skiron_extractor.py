...
class SkironExtractor:
    ...
    def grib2dict_for_train_cfgrib(self, dates):
        results = Parallel(n_jobs=self.n_jobs)(delayed(self.nwps_extract_for_train_cfgrib)(t) for t in dates)
        for res in results:
            if res[1] != "Done":
                self.exclude_dates = self.exclude_dates.append(pd.DatetimeIndex([res[0]]))
        if len(self.exclude_dates) > 20:
    ...
    def extract_nwps(self):
        if self.is_online:
            self.dates = self.static_data['dates'].round('D').unique()
        else:
            self.dates = self.static_data['dates'].round('D').unique() if not self.is_for_test \
                else self.static_data['dates_eval'].round('D').unique()
        dates = []
