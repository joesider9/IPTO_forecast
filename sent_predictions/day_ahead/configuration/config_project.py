...
import numpy as np
import pandas as pd
from day_ahead.configuration.config_utils import *

DOCKER = False
if not ('smartrue' in os.getcwd() or 'joesider' in os.getcwd()):
    DOCKER = True
PROJECT_NAME = 'lv_load'
PROJECT_GROUP = 'IPTO_ver6'
PROJECT_OWNER = 'my_projects'

HORIZON_TYPE = 'day-ahead'  # day-ahead, multi-output, intra-ahead
...
RATED_POWER = None
TYPE = 'load'
FILE_NAME = f"{find_pycharm_path()}/IPTO_ver6/lv_load/data/lv_load_ts.csv"
EVALUATION_START_DATE = '2023-03-01 00:00'  # '%Y-%m-%d %H:%M or None
...
TIME_OFFSET = 0 if TYPE != 'load' else 300
