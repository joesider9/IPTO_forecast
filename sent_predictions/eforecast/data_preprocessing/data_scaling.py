...
class Scaler:
    ...
    def inverse_transform_data(self, data, dataset_name):
        ...
        if not isinstance(data, pd.DataFrame):
            raise ValueError('Cannot inverse transform data. Data should be dataframe')
        index = data.index
        columns = data.columns
        data = data.values
        data_transformed = self.scalers[f'scaler_{dataset_name}'].inverse_transform(data)
        return pd.DataFrame(data_transformed, index=index, columns=columns)
